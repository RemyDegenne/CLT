...
On $K$, the Stone-Weierstrass theorem \texttt{ContinuousMap.starSubalgebra\_topologicalClosure\_eq\_top\_of\_separatesPoints} gives that a subalgebra that separates points is dense. There exists then $f \in \mathcal M$ such that $\left\vert \mu[f \mathbb{I}_K] - \mu[g \mathbb{I}_K] \right\vert \le \varepsilon$ and $\left\vert \nu[f \mathbb{I}_K] - \nu[g \mathbb{I}_K] \right\vert \le \varepsilon$.
...
\begin{proof}\uses{def:cvg_distribution, lem:ext_charFun, lem:tight_of_tendsto_charFun}
