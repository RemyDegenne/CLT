...
\begin{proof}\uses{lem:introduce_exponential, lem:integral_restrict_compact, lem:exponiential_M_eq_limit_M}
Let $K$ be a compact such that $\mu(K^c) \le \varepsilon$ and $\mu_n(K^c) \le \varepsilon$ for all $n$, which exists since the family is tight.

TODO: similar to the proof of Lemma~\ref{lem:Cb_eq_of_separating}.
\end{proof}
